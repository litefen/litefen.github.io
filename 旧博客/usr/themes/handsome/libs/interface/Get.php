 ...
 * @return array
 */
function parseMusicUrlText($url,$size="large",$autoplay){
...
function parseMusicUrl($url,$size="large",$autoplay){
    $ret = parseMusicUrlText($url,$size,$autoplay);

    if ($ret["media"] === ""){
        echo "\n".'[hplayer title="歌曲名" author="歌手" url="'.$ret["url"].'" size="'.$ret["size"].'" '
            .$ret["audoplayHtml"].' /]'."\n";
    }else{
        echo "\n".'[hplayer  media="'.$ret["media"].'" id="'.$ret["id"].'" type="'.$ret["type"].'" size="'.$ret["size"].'" '.$ret["audoplayHtml"].' /]'."\n";
    }
}
